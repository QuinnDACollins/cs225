...
size_t insertion_helper(const std::vector<T>& elements, const C& val, int left, int right) {
    size_t rightSize = size_t(right);
    size_t leftSize = size_t(left);
    size_t midSize = size_t((right + left) / 2);
    if (val == elements[midSize]) {
        return midSize;
    }
    if (elements[midSize + 1] > val ) {
        if (elements[midSize] < val) {
            return midSize + 1;
        }
    }

    if (elements[midSize] < val) {
        return insertion_helper(elements, val, midSize + 1, rightSize);
    }

    return insertion_helper(elements, val, leftSize, midSize);
...
size_t insertion_idx(const std::vector<T>& elements, const C& val)
{
    ...
    return insertion_helper(elements, val, 0, s - 1);
}
