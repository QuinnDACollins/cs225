...
void HuffmanTree::buildTree(const vector<Frequency>& frequencies)
{
     ...
     */
     for(unsigned i = 0; i < frequencies.size(); i++){
         if(frequencies.at(i).getCharacter() != '\0'){
            TreeNode* leafNode = new TreeNode(frequencies.at(i));
            singleQueue.push(leafNode);
         }
     }

     while((mergeQueue.size() + singleQueue.size()) > 1){
         std:: cout << (mergeQueue.size() + singleQueue.size()) << std::endl;
         TreeNode * one = removeSmallest(singleQueue, mergeQueue);
         TreeNode * two = removeSmallest(singleQueue, mergeQueue);
         int sum = one->freq.getFrequency() + two->freq.getFrequency();
         TreeNode * connector = new TreeNode(Frequency(sum));
         connector->left = one;
         connector->right = two;
         mergeQueue.push(connector);
     }
