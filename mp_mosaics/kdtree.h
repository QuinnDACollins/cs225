...
class KDTree
{
  ...
  private:
     ...
     */
    double getDist(const Point<Dim>& first, const Point<Dim>& second) const;

    KDTreeNode* decompose(int n, int d, const vector<Point<Dim>>& p);
    int partition (vector<Point<Dim>>& arr, int dim, int low, int high);
    void quickSort(vector<Point<Dim>>& arr, int dim, int low, int high);
};
